...
class FunsdDataset(Dataset):
    def __init__(self, args, tokenizer, labels, pad_token_label_id, mode):
        ...
        # print([f.input_ids for f in self.features])
        # exit()
    ...
    def __getitem__(self, index):
        # print(np.array(self.all_input_ids[index]))
        # print(np.array( self.all_bboxes[index][index]))
        # print(np.array(self.all_input_ids[index]))
        # print(np.array(self.all_input_ids[index]))
        # exit()
        return (
...
dataset = FunsdDataset(args, tokenizer, labels, pad_token_label_id, mode="train")
...
train_sampler = RandomSampler(train_dataset[:train_split])
train_dataloader = DataLoader(train_dataset[:train_split],
                              sampler=train_sampler,
                              batch_size=2)

val_dataset = FunsdDataset(args, tokenizer, labels, pad_token_label_id, mode="train")
val_sampler = RandomSampler(val_dataset[train_split:])
val_dataloader = DataLoader(val_dataset[train_split:],
                              sampler=val_sampler,
                              batch_size=2)
